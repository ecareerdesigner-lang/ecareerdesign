...
Draw only on details actually present in the candidate's background information below (work experience, education, training). Do not invent specifics (names, numbers, dates) that weren't provided. Use accurate terminology from the candidate's field where the candidate has supplied it. You may naturally mention a job title or timeframe as part of describing the situation (e.g. "As a Program Analyst, I..."), but do so briefly and only where it genuinely helps the narrative — do not open with a formal restatement of the job title and dates as a label or header.

Do NOT restate, quote, paraphrase, or label the requirement itself anywhere in your response (no "Requirement:", no repeating the requirement text back, no headers of any kind). Every character counts toward a hard length limit, so nothing except the narrative answer itself should be in your output.

Requirement: ${requirementText}
Candidate background: ${JSON.stringify(background)}
Target length: aim for about ${target} characters. Do not exceed ${budget} characters under any circumstances — this is a hard limit that includes everything you output. Return ONLY the narrative response text — no preamble, no headers, no restated requirement, no labels of any kind.`;
}
...
The position title, employer, location, and dates below are given for CONTEXT ONLY, so you understand the role. Do NOT restate them in your output — they are already displayed separately as a heading directly above this description, so repeating them wastes space and duplicates what the reader already sees. Your response should begin directly with what the candidate actually did in the role (responsibilities, actions, results) — not a restated job title, employer, location, or date range, and not a label of any kind.
...
Do not exceed ${WORK_EXP_BUDGET} characters under any circumstances — this is a hard limit that includes everything you output. Return ONLY the description text — no preamble, no restated position title/employer/location/dates, no labels of any kind.`;
}
